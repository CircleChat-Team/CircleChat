...
function insertAccount(acc) {
  const now = Date.now();
  open().prepare('INSERT INTO users (name, pass, created, role, settings, updated, status) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(acc.name, hashPassword(acc.pass), now, acc.role || 'user', '{}', now, acc.status || STATUS.ACTIVE);
}
