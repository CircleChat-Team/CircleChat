...
function init(forceDefaults) {
  ...
  if (!forceDefaults) {
    migrateFromJson();
    if (open().prepare('SELECT COUNT(*) AS c FROM users').get().c > 0) return loadUsers();
  }
  // 强制重建默认账号
  const defaults = { 'Ctoy': 'Yhc061900', 'system_mini': 'SYSTEM_mini1234' };
  open().prepare('DELETE FROM users').run();
  const ins = open().prepare('INSERT INTO users (name, pass, created, role, settings, updated) VALUES (?, ?, ?, ?, ?, ?)');
  const now = Date.now();
  for (const name of Object.keys(defaults)) {
    ins.run(name, hashPassword(defaults[name]), now, 'user', '{}', now);
  }
  return loadUsers();
}
