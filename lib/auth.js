...
function getTotp(name) {
  const r = open().prepare('SELECT totp_secret, totp_enabled FROM users WHERE name = ?').get(name);
...
function setTotpEnabled(name, enabled, secret) {
  const r = open().prepare('SELECT 1 AS x FROM users WHERE name = ?').get(name);
  if (!r) return false;
...
function renameUser(oldName, newName) {
  ...
  try {
    ...
    // dm 形如 'a:b'（规范化排序），替换后重新排序保证一致性
    const dmRows = d.prepare('SELECT idx, dm FROM messages WHERE dm IS NOT NULL').all();
...
function submitRegistration(name, password, email) {
  if (!name || !password) return false;
  if (open().prepare('SELECT 1 AS x FROM users WHERE name = ?').get(name)) return false;
