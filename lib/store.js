...
function add(msg) {
  ...
  d.prepare('INSERT INTO messages (idx, id, "from", type, content, ts, name, size, reply_to) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
...
const SELECT_COLS = 'idx, id, "from", type, content, ts, name, size, recalled, recalled_by, recalled_at, file_expired, reply_to';

/** 获取全部（按 idx 正序）；含已撤回 / 已过期的消息，并附上表情回应与引用摘要 */
function all() {
...
module.exports = { load, add, all, get, recall, cleanupExpired, toggleReaction, countByUser, MAX_MESSAGES, DB_FILE, MSGS_FILE: DB_FILE };
