...
function referencedFiles() {
  const set = new Set();
  const rows = open().prepare("SELECT content, type FROM messages WHERE type IN ('image', 'file')").all();
  for (const r of rows) {
...
/** 获取全部（按 idx 正序） */
function all() {
  const rows = open().prepare('SELECT idx, id, "from", type, content, ts, name, size FROM messages ORDER BY idx ASC').all();
...
/** 按 idx 取单条消息（撤回前校验归属） */
function get(idx) {
  const row = open().prepare('SELECT idx, id, "from", type, content, ts, name, size FROM messages WHERE idx = ?').get(idx);
  if (!row) return null;
  const o = { idx: row.idx, id: row.id, from: row.from, type: row.type, content: row.content, ts: row.ts };
  if (row.name != null) o.name = row.name;
  if (row.size != null) o.size = row.size;
  return o;
}

/** 撤回（删除）一条消息；若为图片/文件且不再被引用则清理硬盘文件 */
function recall(idx) {
  const d = open();
