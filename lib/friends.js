...
function sendRequest(from, to) {
  ...
  if (from === to) return { ok: false, reason: '不能添加自己为好友' };
  if (isFriend(from, to)) return { ok: false, reason: '你们已经是好友' };
  const d = open();
  const dup = d.prepare(
    'SELECT 1 AS x FROM friend_requests WHERE status = ? AND requester = ? AND target = ?'
  ).get(REQ_PENDING, from, to);
  if (dup) return { ok: false, reason: '申请已发送，等待对方处理' };
  const rev = d.prepare(
    'SELECT 1 AS x FROM friend_requests WHERE status = ? AND requester = ? AND target = ?'
  ).get(REQ_PENDING, to, from);
  if (rev) return { ok: false, reason: '对方已向你发送申请，请先处理对方的申请' };
