...
(function () {
    ...
    function updateChatTitle(gname) {
        var label;
        if (activeDmPeer != null) label = tr('chat.dm.title', { name: activeDmPeer });
        else if (activeGid != null) label = gname || tr('chat.group.untitled');
        else label = 'ChatPlus';
    ...
    function uploadFiles(files) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (gating && !/^image\//.test(f.type || '')) { toast(tr('chat.dm.gateToast')); continue; }
